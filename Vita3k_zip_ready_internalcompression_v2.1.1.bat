...
title VITA3K READY - WINDOWS ZIP
color 0B
...
echo ZIP engine        : Windows PowerShell Compress-Archive
...
for /f "delims=" %%Z in ('dir /b /a:-d "*_Vita3K_Ready.zip" 2^>nul') do call :ProcessExistingRawZip "%%~nxZ"
...
:: ------------------- STEP 3: ZIP (Windows Compress-Archive) -------------------
echo [STEP 3] ZIP compression with Windows built-in ZIP
echo [FILE] !RAWZIP!
...
if exist "addcont" (
    powershell -NoProfile -ExecutionPolicy Bypass -Command "Compress-Archive -Path 'app','addcont' -DestinationPath '..\!RAWZIP!' -Force" >nul 2>&1
) else (
    powershell -NoProfile -ExecutionPolicy Bypass -Command "Compress-Archive -Path 'app' -DestinationPath '..\!RAWZIP!' -Force" >nul 2>&1
)
set "WERR=!errorlevel!"
popd >nul 2>nul

echo [DEBUG] Windows ZIP exit code for !TITLEID!: !WERR!
...
if not "!WERR!"=="0" (
    >>"%ERRORLOG%" echo !TITLEID! - WINDOWS ZIP ERROR !WERR!
    set "ZIPRES=ZIP_FAIL"
    goto Cleanup
)
...
if exist "!TMPDIR!" (
    ...
    if exist "!TMPDIR!" >>"%ERRORLOG%" echo !TITLEID! - TEMP CLEANUP ERROR
)

if not defined ZIPRES set "ZIPRES="
if not defined RENAMERES_APP set "RENAMERES_APP="
...
endlocal & (
    set "RET_RENAME_RAW_RES=%RENAME_RAW_RES%"
)

if /I "%RET_RENAME_RAW_RES%"=="OK" set /a RENAME_RAW_OK+=1
if /I "%RET_RENAME_RAW_RES%"=="SKIP" set /a RENAME_RAW_SKIP+=1
if /I "%RET_RENAME_RAW_RES%"=="FAIL" set /a RENAME_RAW_FAIL+=1
