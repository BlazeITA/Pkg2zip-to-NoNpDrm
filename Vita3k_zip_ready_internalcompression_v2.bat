...
"$src = Join-Path (Get-Location) '!TMPDIR!';" ^
"$dst = Join-Path (Get-Location) '!RAWZIP!';" ^
"if (Test-Path -LiteralPath $dst) { Remove-Item -LiteralPath $dst -Force };" ^
"Compress-Archive -Path (Join-Path $src '*') -DestinationPath $dst -CompressionLevel Optimal -Force" >nul 2>&1
...
if not "!WERR!"=="0" (
    echo [ERROR] Windows ZIP returned exit code !WERR!
